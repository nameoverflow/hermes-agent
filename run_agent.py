...
class AIAgent:
    ...
    def __init__(
        ...
        # raw_codex=True because the main agent owns raw
        # responses.create(stream=True) event aggregation for Codex.
    ...
    def _recover_codex_stream_response(
        self,
        *,
        ...
        if collected_text_deltas and not has_tool_calls:
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        ...
        Use the raw ``responses.create(stream=True)`` server-event stream as
        the canonical source. The SDK's higher-level ``responses.stream()``
        accumulates events by parsing the terminal ``response.completed``
        snapshot; Codex can send that snapshot with ``output=None`` even
        though the protocol already emitted usable ``response.output_item.done``
        events. Collecting the raw events ourselves avoids depending on that
        terminal parser invariant.
        """
        import httpx as _httpx
        ...
        for attempt in range(max_stream_retries + 1):
            ...
            try:
                ...
                if attempt < max_stream_retries:
                    logger.debug(
                        "Codex raw Responses stream transport failed (attempt %s/%s); retrying. %s error=%s",
                        attempt + 1,
                        max_stream_retries + 1,
                        self._client_log_context(),
                        exc,
                    ...
                    continue
                logger.debug(
                    "Codex raw Responses stream transport failed after retries. %s error=%s",
                    self._client_log_context(),
                    exc,
                )
                raise
                ...
                if missing_completed and attempt < max_stream_retries:
                    logger.debug(
                        "Raw Responses stream closed before completion (attempt %s/%s); retrying. %s",
                        attempt + 1,
                        max_stream_retries + 1,
                        self._client_log_context(),
                    )
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        """Read the raw Responses server-event stream and assemble a final response."""
        ...
        try:
            for event in stream_or_response:
                ...
                if event_type == "response.output_item.done":
                    ...
                    done_item_type = getattr(done_item, "type", None)
                    if done_item_type is None and isinstance(done_item, dict):
                        done_item_type = done_item.get("type")
                    if done_item_type == "function_call":
                        has_tool_calls = True
                elif event_type in {"response.output_item.added", "response.output_item.delta"}:
                    event_item = getattr(event, "item", None)
                    if event_item is None and isinstance(event, dict):
                        event_item = event.get("item")
                    event_item_type = getattr(event_item, "type", None)
                    if event_item_type is None and isinstance(event_item, dict):
        ...
        if terminal_response is not None:
            return terminal_response
        recovered = self._recover_codex_stream_response(
        ...
        if recovered is not None:
            logger.warning(
                "Codex raw Responses stream ended without a terminal response; "
                "recovered from %d output item(s) and %d text delta(s). %s",
                len(collected_output_items),
                len(collected_text_deltas),
                self._client_log_context(),
            )
            return recovered
        raise RuntimeError("Responses create(stream=True) raw stream did not emit response.completed terminal response.")
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        # raw_codex=True because the main agent owns raw
        # responses.create(stream=True) event aggregation for Codex providers.
        try:
            from agent.auxiliary_client import resolve_provider_client
