...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_delete_message_deletes_existing_feishu_message(self):
        ...
        with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
            deleted = asyncio.run(adapter.delete_message("oc_chat", "om_progress"))
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_group_reply_creates_feishu_thread_without_thread_metadata(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
            result = asyncio.run(
                adapter.send(
                    chat_id="oc_chat",
                    content="hello",
                    reply_to="om_top_level_group_message",
                )
            )
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_dm_reply_does_not_request_feishu_thread(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
