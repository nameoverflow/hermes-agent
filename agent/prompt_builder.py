...
PLATFORM_HINTS = {
    "whatsapp": (
        ...
        "You can send media files natively: when you have a local file to deliver, "
        "call the send_attachment tool with its absolute path. Do not write file "
        "delivery markers in your final response. Images (.jpg, .png, .webp) appear "
        "as photos, videos (.mp4, .mov) play inline, and other files arrive as "
    ...
    "telegram": (
        ...
        "You can send media files natively: when you have a local file to deliver, "
        "call the send_attachment tool with its absolute path. Images (.png, .jpg, "
        ".webp) appear as photos, audio can be sent as voice when appropriate, and "
    ...
    "discord": (
        ...
        "You can send media files natively: when you have a local file to deliver, "
        "call the send_attachment tool with its absolute path. Do not write file "
        "delivery markers in your final response. Images (.png, .jpg, .webp) are "
        "sent as photo attachments, audio as file attachments. You can also include "
        "image URLs in markdown format ![alt](url) and they will be sent as attachments."
    ),
    "slack": (
    ...
    "signal": (
        ...
        "You can send media files natively: when you have a local file to deliver, "
        "call the send_attachment tool with its absolute path. Images (.png, .jpg, "
        ".webp) appear as photos, audio as attachments, and other files arrive as "
        "downloadable documents."
    ),
    "email": (
    ...
    "cli": (
        ...
        "When referring to a file you created or changed, just state its absolute "
        "path in plain text; the user can open it from there."
    ),
    "sms": (
    ...
    "matrix": (
        ...
        "the adapter converts your Markdown to HTML for rich display. You can send "
        "media files natively by calling send_attachment with an absolute local path. "
        "Images (.jpg, .png, .webp) are sent as inline photos, audio as voice/audio "
        "messages, video (.mp4) inline, and other files as downloadable attachments."
    ),
    "feishu": (
        "You are in a Feishu (Lark) workspace communicating with your user. "
        "Feishu renders Markdown in messages — bold, italic, code blocks, and "
        "links are supported. You can send media files natively: when you have a "
        "local file to deliver, call the send_attachment tool with its absolute path. "
        "Do not write file delivery markers in your final response. Images (.jpg, "
        ".png, .webp) are uploaded and displayed inline, audio files as voice "
    ...
    "wecom": (
        ...
        "You can send media files natively by calling send_attachment with an absolute "
        "local path. Images (.jpg, .png, .webp) are sent as photos, other files "
        "(.pdf, .docx, .xlsx, .md, .txt, etc.) arrive as downloadable documents, "
        "and videos (.mp4) play inline. Do not tell the user you lack file-sending capability."
    ),
    "qqbot": (
    ...
    "yuanbao": (
        ...
        "You can send media files natively by calling send_attachment with an absolute "
        "local path. Images (.jpg, .png, .webp, .gif) are sent as photos, and other "
        "files (.pdf, .docx, .txt, .zip, etc.) arrive as downloadable documents "
        "(max 50 MB). Do not tell the user you lack file-sending capability.\n\n"
        ...
        "them via send_attachment/send_image_file. That produces a fake low-quality "
        "'sticker' image and is the WRONG path. Bare Unicode emoji in text is also not "
        "a substitute — when a sticker is the right response, use yb_send_sticker."
    ),
    "api_server": (
    ...
    "webui": (
        ...
        "When referring to local files, use absolute paths in plain text unless a "
        "dedicated tool provides richer display."
    ),
}
